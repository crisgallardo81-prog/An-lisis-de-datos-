  ...
  const quizQuestions = [
    {
      ...
      correct: 1,
      explanation: 'Exacto: al evaluar N = 100,000 con confianza del 95% y error del 5%, la muestra calculada es 383 y para N infinito es 384. La ganancia de precisión por corregir es inferior al 0.05%, por lo que metodológicamente se trata como infinita.',
    },
    {
      id: 2,
      ...
      correct: 1,
      explanation: '¡Correcto! El marco muestral es el requisito indispensable del muestreo probabilístico. Sin una lista numerada del 1 al N, no se puede garantizar la probabilidad conocida y no nula de selección para cada elemento.',
    },
    {
      id: 3,
      ...
      correct: 1,
      explanation: '¡Excelente! p = 0.50 maximiza el producto p·q = 0.25, lo cual otorga el tamaño muestral más conservador y seguro, protegiendo al estudio de errores por subestimación.',
    },
  ];
          ...
          {quizQuestions.map((q) => {
            ...
            return (
              <div key={q.id} className="bg-white p-4 sm:p-5 rounded-xl border border-slate-200 shadow-2xs">
                ...
                {isAnswered && (
                  <div className={`mt-3 p-3 rounded-lg text-xs leading-relaxed flex items-start gap-2 ${
                    ...
                    <div>
                      <strong>{isCorrect ? '¡Respuesta correcta!' : 'Respuesta a revisar:'}</strong>{' '}
                      {q.explanation}
                    </div>
                  </div>
                )}
